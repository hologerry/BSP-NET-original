...
import mcubes
from bspt import get_mesh
from ops import conv2d, conv2d_nobias, lrelu, linear
from utils import write_ply_triangle, write_ply_polygon, write_ply_point_normal, sample_points_polygon_vox64

# from bspt_slow import digest_bsp, get_mesh, get_mesh_watertight


class BSP_SVR(object):
    def __init__(self, sess, phase, sample_vox_size, is_training=False, prev_ef_dim=32,
                 ef_dim=64, c_dim=256, p_dim=4096, dataset_name='default', checkpoint_dir=None,
        ...
        if os.path.exists(data_hdf5_name):
            ...
            self.data_pixels = np.reshape(data_dict['pixels'][:, :, offset_y:offset_y+self.crop_size, offset_x:offset_x +
                                                              self.crop_size], [-1, self.view_num, self.crop_size, self.crop_size, 1])
        else:
            ...
            for i in range(multiplier):
                for j in range(multiplier):
                    for k in range(multiplier):
                        ...
                        model_float[self.aux_x+i, self.aux_y+j, self.aux_z+k,
                                    :] = np.reshape(model_out, [self.test_size, self.test_size, self.test_size, self.c_dim])
            ...
            for i in range(multiplier):
                for j in range(multiplier):
                    for k in range(multiplier):
                        ...
                        model_float[self.aux_x+i, self.aux_y+j, self.aux_z+k,
                                    :] = np.reshape(model_out, [self.test_size, self.test_size, self.test_size, self.c_dim])
                        model_float_combined[self.aux_x+i, self.aux_y+j, self.aux_z +
                                             k] = np.reshape(model_out_combined, [self.test_size, self.test_size, self.test_size])
    ...
    def load(self, checkpoint_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            counter = int(next(re.finditer("(\d+)(?!.*\d)", ckpt_name)).group(0))  # noqa
