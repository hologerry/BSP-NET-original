...
char_name = 'A'
char_idx = alphabets.index(char_name)
hdf5_file = h5py.File(f"svg_fonts_{char_name}_test.hdf5", 'w')
...
font_idx_hdf5 = 0
font_idx_hdf5_test = 0
used_font_ids = open('used_font_ids_test.txt', 'w')
...
for font_id_name in font_id_names:
    ...
    if check(font_path):
        font_idx_hdf5 += 1
        if 0 <= font_idx_hdf5 < 2000:
            continue
        img = cv2.imread(os.path.join(font_path, f'{char_idx:02d}.png'), 0)  # use 'A' only
        img = np.reshape(img, (img.shape[0], img.shape[1], 1))
        img = 255 - img
        if font_idx_hdf5_test == 0:
            cv2.imwrite('tmp.png', img)
        hdf5_file['pixels'][font_idx_hdf5_test] = img
        font_idx_hdf5_test += 1
        if font_idx_hdf5_test % 100 == 0:
            print(font_idx_hdf5_test)
        used_font_ids.write(font_id + '\n')
        if font_idx_hdf5_test >= num_shapes:
            break
