...
class IMSEG(object):
    def __init__(self, sess, phase, sample_vox_size, is_training=False, ef_dim=32, gf_dim=64, p_dim=256,
    ...
    def build_model(self, phase):
        ...
        if phase == 0:
            ...
            self.loss = self.loss_sp + tf.reduce_sum(tf.abs(self.cw3-1))*1.0 + (tf.reduce_sum(tf.maximum(self.cw2-1, 0)
                                                                                              ) - tf.reduce_sum(tf.minimum(self.cw2, 0)))*1.0
        elif phase == 1:
            ...
            self.loss_sp = tf.reduce_mean((1-self.point_value)*tf.abs(tf.minimum(self.G, 1)-1) + self.point_value*tf.abs(tf.maximum(self.G, 0)))
            self.bmask = tf.cast(self.G2 < 0.01, tf.float32) * \
                tf.cast(tf.reduce_sum(tf.cast(self.G2 < 0.01, tf.float32), axis=2, keepdims=True) > 1, tf.float32)
    ...
    def train(self, config):
        ...
        for epoch in range(0, training_epoch):
            ...
            for idx in range(batch_num):
                ...
                avg_loss_tt += errTT
                avg_num += 1
            print(str(self.sample_vox_size)+" Epoch: [%2d/%2d] time: %4.4f, loss_sp: %.8f, loss_total: %.8f" %
